# Google Flow AI 影片創作完整教學

### [:arrow_forward: 點擊開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/GoogleFlow_課程總覽.html)

---

![Google Flow](https://img.shields.io/badge/Google-Flow-4285f4?style=for-the-badge&logo=google&logoColor=white)
![Veo 3.1](https://img.shields.io/badge/Veo-3.1-34a853?style=for-the-badge)
![License](https://img.shields.io/badge/License-MIT-yellow?style=for-the-badge)

從零開始學習 Google 最強 AI 影片生成工具 **Flow**，結合 Veo 3.1 技術，輕鬆打造電影級影片！

## 課程特色

- 完整中文教學簡報
- 互動式投影片（支援鍵盤/滑鼠操作）
- 含隨堂測驗與解答
- 電影自拍風格實戰教學
- 運鏡過渡技巧大公開

---

## 線上預覽（點擊直接開啟）

| 單元 | 主題 | 連結 |
|:---:|------|:---:|
| **首頁** | 課程總覽 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/GoogleFlow_課程總覽.html) |
| **Part 1** | Google Flow 入門介紹 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/Part1_Flow入門介紹.html) |
| **Part 2** | 基礎操作與文字轉影片 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/Part2_基礎操作與文字轉影片.html) |
| **Part 3** | 進階功能與創意技巧 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/Part3_進階功能與創意技巧.html) |
| **Part 4** | 自拍與電影風格製作 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/Part4_自拍與電影風格製作.html) |
| **Part 5** | 實戰案例與應用 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/Part5_實戰案例與應用.html) |
| **Part 6** | 穿越時空自拍 | [開啟簡報](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/Part6_穿越時空自拍.html) |
| **工具** | 提示詞生成器 | [開啟工具](https://chatgpt3a01.github.io/GoogleFlow-Tutorial/提示詞生成器.html) |

---

## 課程大綱

### Part 1：Google Flow 入門介紹
- 什麼是 Google Flow
- 三大核心優勢：Consistent、Seamless、Cinematic
- 主要功能總覽
- 訂閱方案比較（免費版 / AI Pro / AI Ultra）

### Part 2：基礎操作與文字轉影片
- 創建新專案
- Text to Video 功能詳解
- 提示詞結構五大要素
- 運鏡與構圖描述
- 音訊生成技巧

### Part 3：進階功能與創意技巧
- Frames to Video（影格轉影片）
- Ingredients to Video（素材控制）
- Video Extend（影片延長）
- Insert Object（物件插入）
- Camera Adjustment（攝影機調整）
- Doodling（塗鴉引導）

### Part 4：自拍與電影風格製作
- 電影幕後自拍提示詞模板
- 三種運鏡過渡風格：
  - VIP 高爾夫球車風格
  - 緊張逃跑風格
  - 急速甩鏡轉場
- 人像一致性技巧

### Part 5：實戰案例與應用
- 電影自拍完整工作流程
- 漫威電影之旅案例
- 商業行銷應用
- 積分管理策略
- 常見問題解決

---

## 操作說明

| 操作 | 方式 |
|------|------|
| 下一頁 | 點擊右箭頭 / 按鍵盤 `→` |
| 上一頁 | 點擊左箭頭 / 按鍵盤 `←` |
| 跳轉頁面 | 點擊底部頁碼圓點 |
| 全螢幕 | 點擊導航列「全螢幕」按鈕 |
| 解鎖測驗答案 | 密碼：`0207` |

---

## 相關資源

- [Google Flow 官網](https://labs.google/flow/about)
- [Google Blog - Flow 介紹](https://blog.google/technology/ai/google-flow-veo-ai-filmmaking-tool/)
- [Flow 官方幫助中心](https://support.google.com/labs/answer/16353334)

---

## 授權

本教學內容由 **曾慶良（阿亮老師）** 製作

© 2025 Google Flow 教學 | Made with ❤️
